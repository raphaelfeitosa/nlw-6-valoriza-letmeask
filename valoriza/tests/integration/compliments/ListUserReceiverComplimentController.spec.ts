...
describe("List user receiver controller", () => {
  beforeAll(async () => {
  ...
  it("should be able to list user receiver compliments", async () => {
    ...
    const response = await request(app)
      .get("/api/v1/compliments/receiver")
  ...
  it("should be able to list a second page of received compliments", async () => {
    ...
    delete user_receiver.password;
    const response = await request(app)
      .get("/api/v1/compliments/receiver?page=2")
      .set({ Authorization: `Bearer ${jwtoken(user_receiver.id)}` })
      .expect(200)
      .send();
